...
#export TRAJOKIT_MAX_CONTEXT=32768
#export TRAJOKIT_ROLLOUT_CACHE=/mnt/raid5/rollout_cache
export TRAJOKIT_MAX_CONTEXT=20480          # 4096 prompt + 16384 response
export TRAJOKIT_ROLLOUT_CACHE=/mnt/raid5/rollout_cache_20k   # fresh dir, old cache is 32k-shaped
export TRAJOKIT_CACHE_RUN=$(date +%s)   # new id per replay

#!/bin/bash
# 1-step GSPO smoke: 8B, single GPU, offloaded optimizer (Pro 6000 = device 2)
set -e
CUDA_VISIBLE_DEVICES=2 .venv/bin/python -m verl.trainer.main_ppo \
  ...
  trainer.experiment_name=gspo-8b-train-15step \
